...
3. [Acode X Terminal (Acode Plugin)](https://acode.foxdebug.com/plugins) — download from Acode's plugin directory
...
## 🧭 Acode X Terminal (Plugin Overview & Use Case)

[Acode X Terminal](https://acode.foxdebug.com/plugins) is an Acode plugin providing an integrated terminal inside the Acode editor. It enables you to run shell commands (including Termux commands) in a dedicated terminal tab within Acode, without switching apps.

**Use Cases & Benefits:**
- **Quick command execution:** Run `flutter`, `dart`, or other commands right next to your editor window for fast iteration.
- **Faster edit-run cycle:** Save in Acode and run build or hot reload commands in the terminal pane.
- **Easier log access:** See `flutter run` logs inline alongside code when debugging.
- **Smooth permissions/storage access:** Use the terminal to invoke `termux-setup-storage` or other environment setup commands without leaving Acode.

**Note:**  
Acode X Terminal complements Termux. Termux provides a full Android Linux environment; Acode X Terminal offers quick shell access inside the editor. For heavy installs and persistent background tasks (like installing the Flutter SDK), Termux is still recommended.
